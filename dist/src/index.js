...
import { createSet, ensurePlayer, findActiveSetForUser, getSetCount, getPlayer, nowSec, openDb, runMigrations, timeoutSweep, userHasActiveSet, } from "./db.js";
...
function parseScore(s) {
    ...
    if (w <= 0 || l < 0)
        return null;
    if (l >= w)
        return null; // winner must have more wins
    if (w > 9 || l > 9)
        return null;
    return { w, l };
}
...
client.on(Events.MessageCreate, async (msg) => {
    ...
    try {
        if (parsed.cmd === "play") {
            ...
            await msg.reply(`set #${setId} started: ${mention(authorId)} vs ${mention(target.id)}. ` +
        ...
        `).run({ g: guildId, u: loserId, r: elo.loserAfter, ts });
            ...
            try {
                const r = txn();
                await msg.reply(`set #${r.setId} resolved: ${mention(r.winnerId)} won ${score.w}-${score.l} vs ${mention(r.loserId)}. ` +
                ...
                if (m === "NO_ACTIVE")
                    await msg.reply("no active set to report in this channel");
                else if (m === "TIMED_OUT")
                    await msg.reply("that set timed out (2h). start a new one.");
                else
                    throw e;
            }
            return;
